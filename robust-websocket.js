(function (factory, global) {
  if (typeof define === "function" && define.amd) {
    ...
    global.RobustWebSocket = factory(
      global,
      typeof Mocha !== "undefined" ? Mocha : navigator
    );
  }
})(
  function (global, navigator) {
    ...
    var RobustWebSocket = function (url, protocols, userOptions) {
      ...
      ["bufferedAmount", "url", "readyState", "protocol", "extensions"].forEach(
      ...
      self.open = function () {
        if (
          realWs.readyState !== WebSocket.OPEN &&
          realWs.readyState !== WebSocket.CONNECTING
        ) {
          clearPendingReconnectIfNeeded();
          reconnectWhenOnlineAgain = false;
          explicitlyClosed = false;
      ...
      function reconnect(event) {
        if (
          (!opts.shouldReconnect.handle1000 && event.code === 1000) ||
          explicitlyClosed
        ) {
          attempts = 0;
          return;
      ...
      function newWebSocket() {
        var urlFactory = typeof url === "function" ? url(self) : url;

        if (!isPromise(urlFactory)) {
          urlFactory = Promise.resolve(urlFactory);
        }

        urlFactory.then((endpoint) => {
          pendingReconnect = null;
          realWs = new WebSocket(endpoint, protocols || undefined);
